...
{
    mpu6050_dev_t *sens = (mpu6050_dev_t *) sensor;
    esp_err_t ret = i2c_master_transmit(sens->i2c_dev, &reg_start_addr, 1, 1000);
    if (ret != ESP_OK) {
        return ret;
    }
    return i2c_master_receive(sens->i2c_dev, data_buf, data_len, 1000);
}
...
mpu6050_handle_t mpu6050_create(i2c_master_bus_handle_t bus_handle, const uint16_t dev_addr)
{
    ...
    i2c_master_bus_add_device(bus_handle, &dev_config, &sensor->i2c_dev);
